...
- **Region**: us-central1  
- **Runtime**: Node.js 20
- **Trigger**: Firestore document writes in `authDeleteQueue/{uid}`
- **Status**: ACTIVE (deployed November 6, 2025)

### What It Does:
1. Watches for queue documents in `authDeleteQueue/` collection
...
2. **Check Firestore**: Document appears in `authDeleteQueue/{uid}`
...
2. ✅ Queue document created in `authDeleteQueue/`
